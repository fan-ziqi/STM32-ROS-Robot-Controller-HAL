...
void cmd(void)
{
	...
	else if(!strcmp(CMD_Buffer,"show_task"))         {show_task();}
	else
	{
...
void help(void)
{
	...
	printf("show_task            ----------     Show Task Status\r\n");
}
...
//		printf("Paint ID:%d Lable:%s Data:%d Color:%s Style:%s\r\n",1,"Pitch",robot_imu_dmp_data.pitch,"red","SolidLine");
//		printf("Paint ID:%d Lable:%s Data:%d Color:%s Style:%s\r\n",2,"Roll",robot_imu_dmp_data.roll,"green","DashLine");
//		printf("Paint ID:%d Lable:%s Data:%d Color:%s Style:%s\r\n",3,"Yaw",robot_imu_dmp_data.yaw,"blue","DotLine");
